...
- 作る：`gion.yaml` を手動編集、または `gion manifest add` で更新 → `gion apply`（Plan→確認→Apply）
- 移動：`giongo` で検索して移動（状態は変えない）
- 片付け：`gion.yaml` を手動編集、または `gion manifest gc` / `gion manifest rm` で更新 → `gion apply`（Plan→確認→Apply）
