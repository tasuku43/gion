...
## Synopsis
`gion manifest add [--preset <name> | --review [<PR URL>] | --issue [<ISSUE_URL>] | --repo [<repo>]] [<WORKSPACE_ID>] [--branch <name>] [--base <ref>] [--no-apply] [--no-prompt]`

Note: If no mode flag is provided and prompts are allowed, the mode is chosen via an interactive picker.

## Intent
Create the desired workspace inventory in `gion.yaml` using an interactive UX, then reconcile the filesystem via `gion apply` by default.
...
- Produces a workspace definition (mode, description, optional metadata, repo list with alias/repo_key/branch) and writes it to `<root>/gion.yaml`.
- If the target `WORKSPACE_ID` already exists in `gion.yaml`, error (no upsert in MVP).
- If the target workspace already exists on the filesystem (`<root>/workspaces/<WORKSPACE_ID>`) but is missing from `gion.yaml`, error and suggest `gion import` (do not adopt implicitly).
- By default, runs `gion apply` to reconcile the filesystem with the updated manifest.
  - `gion apply` reconciles the entire root (full diff between `gion.yaml` and filesystem), which may include unrelated drift in the same root.
  - Confirmation and destructive rules follow `gion apply` spec.
- `--no-prompt` is forwarded to `gion apply` when apply is run.
  - If the full-root plan contains any removals, `gion apply --no-prompt` must error; `gion manifest add` does not preflight-block this case.
- With `--no-apply`, stops after rewriting `gion.yaml` and prints a suggestion to run `gion apply` (or `gion plan`) next.
...
When `gion manifest add` runs apply (default behavior), it performs a two-phase action:
1) rewrite `gion.yaml`, then
2) run `gion apply` (which includes the confirmation prompt).

If the user cancels at the apply confirmation step (e.g. answers `n`/No) or interrupts at the prompt (`Ctrl-C`), `gion manifest add` should roll back the manifest change by restoring the previous `gion.yaml` content.

Guidance:
- Prefer restoring from a backup of the pre-change `gion.yaml` (or an in-memory snapshot) rather than running `gion import`.
  - A simple approach is to write a temporary backup file before the rewrite, then restore it on cancellation.
- Do not use `gion import` as a rollback mechanism:
...
Notes:
- If apply proceeds past confirmation and starts executing steps, failures are treated as apply failures (the manifest remains updated; users can re-run `gion apply`).
...
4. Collision checks:
   - If `WORKSPACE_ID` exists in `gion.yaml`, error.
   - If `<root>/workspaces/<WORKSPACE_ID>` exists but is missing from `gion.yaml`, error and suggest `gion import`.
5. Rewrite `gion.yaml` (full-file rewrite).
6. If `--no-apply` is set: stop after manifest rewrite.
7. Otherwise run `gion apply` for the entire root:
   - This may include unrelated drift in the same root.
   - Confirmation and destructive rules are handled by `gion apply`.
8. If apply is cancelled at the confirmation step, restore the previous `gion.yaml`.
...
- If `--base <ref>` is provided, it must be in the form `origin/<branch>`, and `gion manifest add` writes it as `base_ref` into the corresponding repo entry in `gion.yaml`.
  - `base_ref` is used only when the branch does not already exist in the bare store.
  - If `base_ref` does not resolve when it is needed, `gion apply` fails (manifest remains updated).
...
This command stores the target branch per repo as `repos[].branch` in `gion.yaml`. When `gion apply` materializes the workspace, each repo worktree is checked out to that branch.
...
- In multi-select mode:
  - The command writes all selected workspaces into `gion.yaml` in one run.
  - Then, by default, runs `gion apply` once (single plan + single confirmation) for the updated manifest.
...
### Output: `--no-apply`
When `--no-apply` is set, `gion manifest add` does not run apply and prints a short summary instead.
...
`gion manifest add` should keep errors actionable and include the next command when possible.
...
- Workspace exists on filesystem but missing in manifest: error and suggest `gion import`.
- Apply fails after manifest rewrite:
  - Treat as apply failure and keep the manifest change (users can re-run `gion apply`).

## Success Criteria
- `gion.yaml` contains the intended workspace entry in normalized form.
...
- `gion apply` failure (git/filesystem).
  - When apply fails after manifest write, the manifest remains updated; users can re-run `gion apply`.
