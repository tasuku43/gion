...
- GitHub Release artifacts:
  - `gion_vX.Y.Z_macos_arm64.tar.gz`
  - `gion_vX.Y.Z_macos_x64.tar.gz`
  - `gion_vX.Y.Z_linux_arm64.tar.gz`
  - `gion_vX.Y.Z_linux_x64.tar.gz`
  - `checksums.txt` (SHA256)
- Homebrew Formula: `Formula/gion.rb`
...
flowchart LR
  ...
  Updater --> PR[PR updating Formula/gion.rb]
...
- `gion version` correctness is guaranteed for **GitHub Releases binaries** by `-ldflags` injected by GoReleaser.
