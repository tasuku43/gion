# AGENTS.md — gion repository instructions

## Project summary
- Project: gion (Git Workspace Manager)
- Language: Go
...
- gion uses an IaC-style workflow centered on **Declare → Diff → Reconcile**.
- `gion.yaml` is the workspace **desired state (inventory)**:
  - Location: `<GION_ROOT>/gion.yaml`
  - Inventory is updated via `gion manifest ...`.
  - `gion apply`: `gion.yaml` is the source of truth; gion computes a diff and reconciles the filesystem.
...
- For manifest mutation commands (`gion manifest add/rm/gc/...`):
...
- When creating issues or PRs with `gh`, pass the body via a here-doc to preserve newlines for proper GitHub rendering:
  ```sh
  gh issue create \
    --title "Update gion command surface" \
    --body "$(cat <<'EOF'
  ## Summary
  ...
  EOF
